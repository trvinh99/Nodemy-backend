...
const isObjectId = (id = '', name = '') => {
  const error = `Format of ${name}'s is invalid!`;
...
const checkType = (data, type = '', name = '') => {
  ...
  if (typeof data !== type) {
    throw new NodemyResponseError(400, `Type of ${name} must be ${type}!`);
  }
};
...
const stringConstraints = (string = '', name = '', constraints = {
  ...
  if (typeof constraints.minLength === 'number' && constraints.minLength >= 0) {
    if (str.length < constraints.minLength) {
      throw new NodemyResponseError(400, `${name} must not contain less than ${constraints.minLength} characters!`);
    }
  }

  if (typeof constraints.maxLength === 'number' && constraints.maxLength >= 0) {
    if (str.length > constraints.maxLength) {
      throw new NodemyResponseError(400, `${name} must not contain more than ${constraints.maxLength} characters!`);
    }
  }

  if (constraints.isRequired && !str) {
    throw new NodemyResponseError(400, `${name} is required!`);
  }
};
...
const numberConstraints = (number = '', name = '', constraints = {
  ...
  if (typeof constraints.min === 'number' && number < constraints.min) {
    throw new NodemyResponseError(400, `${name} must not less than ${constraints.min}!`);
  }

  if (typeof constraints.max === 'number' && number > constraints.max) {
    throw new NodemyResponseError(400, `${name} must not greater than ${constraints.max}!`);
  }
};

const objectConstraints = (object = {}, name = '', requiredProps = []) => {
  ...
  Object.keys(object).forEach((prop) => {
    if (!requiredProps.includes(prop)) {
      throw new NodemyResponseError(400, `Format of ${name}'s body is invalid!`);
    }
  });
};
